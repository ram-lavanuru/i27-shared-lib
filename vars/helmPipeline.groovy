...
def imageValidation() {
    return {
        ...
        try {
        sh "docker pull ${env.JFROG_DOCKER_REGISTRY}/${JFROG_REPO_NAME}/${env.APPLICATION_NAME}:${GIT_COMMIT}"
...
def dockerBuild() {
    ...
    docker build --no-cache --build-arg JAR_SOURCE=i27-${env.APPLICATION_NAME}-${env.POM_VERSION}.${env.POM_PACKAGING} -t ${env.JFROG_DOCKER_REGISTRY}/${JFROG_REPO_NAME}/${APPLICATION_NAME}:${GIT_COMMIT} ./.cicd
    echo "********login to doker registry****"
    docker login -u ${JFROG_CREDS_USR} -p ${JFROG_CREDS_PSW} i27devopsd4.jfrog.io
    docker push ${env.JFROG_DOCKER_REGISTRY}/${JFROG_REPO_NAME}/${env.APPLICATION_NAME}:${GIT_COMMIT}
    """
    }
}
